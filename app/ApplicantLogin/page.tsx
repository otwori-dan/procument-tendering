...
export default function Component() {
   const router = useRouter()
    ...
    const [password, setPassword] = useState('');
      ...
      const handleSubmit = async (e: { preventDefault: () => void; }) => {
       ...
       try {
        let res = await fetch('/api/auth/applicant/signin', {
          method: 'POST',
          headers: {
        ...
        if(response.message === 'ok'){
          ...
          //applicantId
          localStorage.setItem('applicantId', JSON.stringify(response.user.id));
  ...
  return (
    ...
    <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
      <h2 className="text-3xl font-bold text-center text-gray-900 dark:text-gray-100">Sign In As Applicant</h2>
